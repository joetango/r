...
data <- read.csv("Bailey2008_B6xC58J_B37_Data.csv")

cross0 <- sim.cross(data, type="f2", n.ind=100, model = rbind(c(1,50,4,0))) 
...
data <- read.cross(
  format = "csv",
  file = "Bailey2008_B6xC58J_B37_Data.csv",
  genotypes = c("A","H","B"),
  na.strings = c("-", "NA"),
  alleles = c("A","B"),
  estimate.map = FALSE
...
scanone(mydata)
out.chol <- scanone(mydata, pheno.col="Chol", method="hk")
plot(out.chol)
